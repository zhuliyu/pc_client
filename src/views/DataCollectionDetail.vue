<template>
  <div class="data-panel">
    ...
    <div class="panel-container">
      <div class="item-list">
        <div class="item-left">
          <div class="left-title">{{info.machineName}}</div>
          <div class="left-btn offline" :style="{background: colorMatch[info.equipmentStatus]}">{{nameMatch[info.equipmentStatus]}}</div>
        </div>
        <div class="item-center">
          <div class="center-list center-equipment">
            ...
            <div class="center-list-code">{{info.machineId}}</div>
          </div>
          <div class="center-list">
            ...
            <div class="center-list-code">{{info.updateTime}}</div>
          </div>
        </div>
...
export default class DataCollectionDetail extends Vue {
  ...
  timeList:any = [
    { color: '#0cdbff', title: '运行', status: 'online', time: '' },
    { color: '#0bce8a', title: '待机', status: 'standby', time: '' },
    { color: '#a8b4c6', title: '离线', status: 'offline', time: '' },
    { color: '#f2a21b', title: '警告', status: 'warning', time: '' }
  ]
  eqpStatusList: any = []
  colorMatch: any = {
    'online': '#6bfffd',
      ...
      .then((res:any) => {
        if (res.code === 200) {
          if (res.data && res.data.length) {
            this.info = res.data.find((item:any) => item.machineName === title)
            this.handleDate()
          }
        }
      })
  ...
  handleDate () {
    ...
    this.info.allStatusArr && this.info.allStatusArr.length && this.info.allStatusArr.map((item:any, index:any) => {
      let nowTime:any = this.getTime(this.info.allTimeArr[index])
      let prevTime:any = 0
      if (this.info.allTimeArr[index - 1]) {
        prevTime = this.getTime(this.info.allTimeArr[index - 1])
      }
